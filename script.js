...
async function loadProperties(filters = {}) {
          ...
          <div class="card-details">
            <p class="location">${property.location}</p>
             <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.8rem;">
               ...
               <span class="property-type-badge">${property.type || 'Apartment'}</span>
             </div>
  ...
  ${currentUser && currentUser.role === "owner"
          ...
          : property.status === 'rented'
            ? `<button class="book-now booked" disabled>Booked</button>`
...
window.openPropertyModal = async function (propertyId) {
  ...
  try {
    ...
    if (isOwner) {
      ...
      // Property is already booked - show disabled button
      ...
      // Property is available - show Book Now button
